...
describe('fileParser', () => {
  ...
  describe('readFile', () => {
    ...
    it('should read Excel files', async () => {
      ...
      expect(result.data).toEqual(mockData);
      expect(result.errors).toHaveLength(0);
      expect(result.debug.excelMeta).toEqual({
        sheetName: 'Sheet1',
        range: 'A1:G1',
        dimensions: { r: 1, c: 7 }
      });
    ...
    it('should read jobs_constrained.csv correctly', async () => {
      ...
      expect(result.data.length).toBe(51); // Number of jobs in test file
      expect(result.errors).toHaveLength(0);
    ...
    it('should read salesmen.csv correctly', async () => {
      // Read the test file content
      const filePath = path.join(process.cwd(), 'test', 'salesmen.csv');
      const fileContent = fs.readFileSync(filePath, 'utf-8');
      ...
      expect(result.data).toBeDefined();
      expect(result.data.length).toBe(10); // Number of salesmen in test file
      expect(result.errors).toHaveLength(0);
  ...
  describe('parse', () => {
    it('should parse jobs_constrained.csv data correctly', async () => {
      const filePath = path.join(process.cwd(), 'test', 'jobs_constrained.csv');
      const fileContent = fs.readFileSync(filePath, 'utf-8');
      const rawData = Papa.parse(fileContent, { header: true }).data;
